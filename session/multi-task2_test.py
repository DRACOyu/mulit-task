...
def deeplab_v3_plus_generator(inputs, labels_nums,labels_nums_seg,is_training):
      ...
      with tf.contrib.slim.arg_scope(inception_v3_arg_scope()):
        with arg_scope([layers.batch_norm], is_training=True):
          with tf.variable_scope('Logits'):
                ...
                kernel_size = [14, 3]
                net_reid_test = slim.avg_pool2d(net, kernel_size, padding='VALID',
                                      scope='AvgPool_1a_{}x{}'.format(*kernel_size))

                logits = slim.conv2d(net_reid_test, labels_nums, [1, 1], activation_fn=None,
    ...
    return logits,logits_reid,net_reid_test
...
def Evaluation(sess,net_reid_test):
    ...
    for f in F:
        ...
        im = im[np.newaxis, :]
        feature = sess.run(net_reid_test, feed_dict={input_images: im})
    ...
    for f in G:
        ...
        im1 = im1[np.newaxis, :]
        feature1 = sess.run(net_reid_test, feed_dict={input_images: im1})
...
def train(train_record_file,
    ...
    logits, logits_reid,net_reid_test = deeplab_v3_plus_generator(input_images,labels_nums,labels_nums_seg,is_training)
    ...
    train_op_seg = slim.learning.create_train_op(total_loss=loss_seg, optimizer=optimizer)
    ...
    exclusions = ["decoder",'InceptionV3/dalated_reduction7', 'InceptionV3/dalated_inception7']

    inception_except_logits = slim.get_variables_to_restore(exclude=exclusions)


    saver = tf.train.Saver(inception_except_logits,max_to_keep=10)
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_steps + 1):
            ...
            if i % 2 ==0:
                ...
                if i % train_log_step == 0:
                    ...
                    train_acc = sess.run(accuracy, feed_dict={input_images: batch_input_images,
                                                              input_labels: batch_input_labels,
                                                              is_training: False})
                    print("%s: Step [%d]  train Loss : %f,training accuracy :  %g val Loss : %f, val accuracy :  %g" % (
                        datetime.now(), i, train_loss, train_acc,mean_loss, mean_acc))
            else:
            ...
            if i>=30000 and  i%10000 ==0:
                print("--------------------------------eval----------------------------------------------")
                map,r  =  Evaluation(sess,net_reid_test)
                print('mAP=%f, r@1=%f, r@10=%f, r@25=%f, r@50=%f, r@100=%f' % (
...
if __name__ == '__main__':
    ...
    train_log_step=2000
    base_lr = 0.01  # 学习率
    max_steps = 300000  # 迭代次数
    train_param=[base_lr,max_steps]

    val_log_step=2
    snapshot=2000#保存文件间隔
    snapshot_prefix='/yzc/test/test_module/models-multi-0119-eval/model'
